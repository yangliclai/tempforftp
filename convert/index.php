...
/**
 *      Convert Chinese Discuz!X to ANOTHER language
 *      by Valery Votintsev, http://discuz.ml
 *	The target language is defined in data/config.php
 *	i.e. zh_cn, zh_tw, en, ru, etc.
 */

//-----------------------------------------------

global $config, $lng, $lng_code;

error_reporting(E_ALL);

define('THIS_PATH', str_replace('\\', '/', dirname(__FILE__)));

define('ROOT_PATH', dirname(THIS_PATH));

//Define APPTYPEID, CURSCRIPT
define('APPTYPEID', 2);
define('CURSCRIPT', 'convert');
...
$_G['siteurl'] = preg_replace('/\/'.CURSCRIPT.'\/$/i', '/', $_G['siteurl']);


//------------------------------------------------------

include_once(THIS_PATH . '/data/config.php');

//DEBUG
//echo '<pre>';
//echo 'lng=', $lng, "\n";
//echo 'lng_code=', $lng_code, "\n";
//echo '</pre>';
...
if(file_exists($lockfile) && !$_GET['from']) {
	show_msg('You must delete the "/data/update.lock" file manually and then reload this page.');
}

$sqlfile = THIS_PATH . '/convert.sql';
...
if($_POST['delsubmit']) {
	...
	show_msg('Deleting the table and the fields is completed', $theurl.'?step=style');
}
...
function waitingdb($curstep, $sqlarray) {
	...
	show_msg('Optimize table', $theurl.'?step=waitingdb&nextstep='.$curstep.$sqlurl.'&sendsql='.base64_encode($sendsql), 5000, 1);
}
...
if($_GET['step'] == 'start') {
	...
	if(!C::t('common_setting')->fetch('bbclosed')) {
		...
		updatecache('setting');
		show_msg('Your site is not closed, is being closed, please wait...', $theurl.'?step=start', 5000);
	}
	if(version_compare($version, '1.5.2') <= 0) {
		show_msg('Please upgrade your UCenter to v.1.6.0 or later.<br>If you use the Discuz!X comes with UCenter, please upgrade UCenter by running the uc_server/upgrade/upgrade3.php');
	} else {
		show_msg('<b>Note:</b>
			<p>
			This program will update your database structure, prepare it to use <b>' .$lng. '</b> language in the database<br>
			and then translate some settings in the database to this language.
			</p>
			<p>
			Before run of this program you need to do the next:
			<ul style="list-style-type: decimal;">
			<li>Backup all the Discuz files.</li>
			<li>Backup your Discuz database.</li>
			<li>Convert your Discuz database to "utf" encoding with "utf8-general_ci" collation.</li>
			<li>Upload all the files and directories from the /uploads/'.$lng.'/ directory
			into the root of your Discuz site with overwriting old files.</li>
			</ul>
			</p>
			<p>
			After the conversion program is completed, you have go to your Admin Control Panel and update all the cache.
			</p>
			<p>
			This program turns off all the plug-ins to ensure the site is working properly.
			<br>
			So you need to enable your plugins one by one for detect if each plug-in is compatible with a new version.
			</p>
			<p>
			<center><a href="'.$theurl.'?step=prepare'.($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '').'">Start the Convert Process</a></center>
			</p>');
	}
...
//---------------------------------------------------------------------
	...
	foreach($query as $row) {
		if(in_array(md5($row['Info']), $_GET['sql'])) {
			$list .= '[Length]: '.$row['Time'].' [Status]: <b>'.$row['State'].'</b> [Info]: '.$row['Info'].'<br><br>';
		}
	}
	if(empty($list) && empty($_GET['sendsql'])) {
		...
		$msg = 'Starting to upgrade, please wait...';
		$notice = '<br><br><b>The following is a database upgrade statement being executed:</b><br>' .$list.base64_decode($_GET['sendsql']);
...
//---------------------------------------------------------------------
	...
	if(DB::fetch_first("SHOW COLUMNS FROM ".DB::table('forum_activityapply')." LIKE 'contact'")) {
		$query = DB::query("UPDATE ".DB::table('forum_activityapply')." SET message=CONCAT_WS(' Contacts: ', message, contact) WHERE contact<>''");
	...
	if($row = DB::fetch_first("SHOW COLUMNS FROM ".DB::table('forum_postcomment')." LIKE 'authorid'")) {
		if(strstr($row['Type'], 'unsigned')) {
			DB::query("ALTER TABLE ".DB::table('forum_postcomment')." CHANGE authorid authorid int(11) NOT NULL default '0'");
	...
	if(!$row = DB::fetch_first("SHOW COLUMNS FROM ".DB::table('common_failedlogin')." LIKE 'username'")) {
		...
		DB::query("ALTER TABLE ".DB::table('common_failedlogin')." ADD username varchar(64) NOT NULL default '' AFTER ip");
	...
	if(!$row = DB::fetch_first("SHOW COLUMNS FROM ".DB::table('forum_forumfield')." LIKE 'seodescription'")) {
		DB::query("ALTER TABLE ".DB::table('forum_forumfield')." ADD seodescription text NOT NULL default '' COMMENT 'SEO forum description' AFTER keywords");
	...
	foreach($posttables as $post_tablename) {
		if(!DB::fetch_first("SHOW COLUMNS FROM ".DB::table($post_tablename)." LIKE 'position'")) {
			...
			$sql[] = "ALTER TABLE ".DB::table($post_tablename)." CHANGE `pid` `pid` INT(11) UNSIGNED NOT NULL,$replycreditsql CHANGE `status` `status` int(11) NOT NULL default '0', ADD UNIQUE KEY pid (pid), DROP PRIMARY KEY, ADD `position` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT, ADD PRIMARY KEY(`tid`, `position`), DROP INDEX authorid, ADD INDEX authorid (authorid,invisible)";
			if(!$_GET['waitingdb']) {
	...
	show_msg('Ready, the next step will upgrade the database structure', $theurl.'?step=sql');
...
//---------------------------------------------------------------------
	...
	if(empty($newtables) || empty($newsqls)) {
		show_msg('SQL file is empty, make sure');
	}

	$i = empty($_GET['i'])?0:intval($_GET['i']);
	$count_i = count($newtables);
	if($i>=$count_i) {
		show_msg('The database structure upgrade is completed, enter to update the next data', $theurl.'?step=data');
	}
	...
	if(!$query = DB::query("SHOW CREATE TABLE ".DB::table($newtable), 'SILENT')) {
		...
		if(!DB::query($usql, 'SILENT')) {
			show_msg('Add table ' .DB::table($newtable). ' Error. Please manually execute the following SQL statement, and then re-run the upgrade program:<br><br>'.dhtmlspecialchars($usql));
		} else {
			$msg = 'Add table ' .DB::table($newtable). ' completed';
		}
	} else {
			...
			if($key == 'PRIMARY') {
				if($value != $oldcols[$key]) {
					if(!empty($oldcols[$key])) {
						...
						if(!DB::query($usql, 'SILENT')) {
							show_msg('Upgrade table ' .DB::table($newtable). ' Error. Please manually execute the following SQL statement, and then re-run the upgrade program:<br><br><b>Upgrading SQL statement</b>: <div style=\"position:absolute;font-size:11px;font-family:verdana,arial;background:#EBEBEB;padding:0.5em;\">'.dhtmlspecialchars($usql)."</div><br><b>Error</b>: ".DB::error()."<br><b>Errno.</b>: ".DB::errno());
						} else {
							$msg = 'Rename Table ' .DB::table($newtable). ' completed';
		...
		if(!empty($updates)) {
			...
			if(!DB::query($usql, 'SILENT')) {
				...
				$msg = 'Upgrade table ' .DB::table($newtable). ' completed';
			}
		} else {
			$msg = 'Check table ' .DB::table($newtable). ' completed, without upgrading, skipped';
		}
	}
...
//---------------------------------------------------------------------
	...
	if(empty($_GET['op']) || $_GET['op'] == 'realname') {
		...
		if($i==0) {
			...
			if(!empty($value)) {
				show_msg('Real name feature upgraded', "$theurl?step=data&op=$nextop");
			}
			DB::query("INSERT INTO ".DB::table('common_member_profile_setting')." VALUES ('realname', '1', '0', '1', 'Real name', '', '0', '0', '0', '0', '1', 'text', '0', '', '', '0', '0')");
		}
		...
		if($n>0) {
			show_msg("Real names upgrading progress: [$n/$t]", "$theurl?step=data&op=realname&i=$n");
		} else {
			show_msg('Real name feature upgraded', "$theurl?step=data&op=$nextop");
		}
		...
		//if($profile['title'] == '出生日期' || $profile['title'] == 'Birthday') {//vot: Chinese/English "Birthday"
			DB::query("UPDATE ".DB::table('common_member_profile_setting')." SET title='Birthday' WHERE fieldid = 'birthday'");
			DB::query("UPDATE ".DB::table('common_member_profile_setting')." SET title='Document type' WHERE fieldid = 'idcardtype'");
...
//vot		}
		show_msg('User section upgrade finished', "$theurl?step=data&op=$nextop");
		...
		if(empty($settings['reglinkname'])) {
			$newsettings['reglinkname'] = 'Registration';
		}
		...
		if(!isset($settings['focus'])) {
			$focusnew = array('title' => 'Recommended by webmaster', 'cookie' => 1);
		...
		if(!isset($settings['article_tags'])) {
			$article_tagsnew = array(1 => 'Original', 2 => 'Hot', 3 => 'Photos', 4 => 'Breaking News', 5 => 'Headline', 6 => 'Slideshow', 7 => 'Scroll', 8 => 'Recommended');
			$newsettings['article_tags'] = $article_tagsnew;
		}
		if(empty($settings['anonymoustext'])) {
			$newsettings['anonymoustext'] = 'Anonymous';
		}
		if(!$word_type_count = DB::result_first("SELECT count(*) FROM ".DB::table('common_word_type')."")) {
			DB::query("INSERT INTO ".DB::table('common_word_type')." VALUES('1', 'Politics'),('2', 'Advertising')");
		}
		if(!isset($settings['userreasons'])) {
			$newsettings['userreasons'] = 'Powerfull!\r\nUsefull\r\nVery nice!\r\nThe best!\r\nInteresting';
		}
		...
		if(empty($settings['memory']) || isset($settings['memory']['diyblock']['enable'])) {
			...
			$newsettings['memory'] = array_merge(array(
							'common_member' => 0,
		...
		show_msg('Configuration, Settings upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Group Settings upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		if(DB::result_first("SELECT name FROM ".DB::table('common_magic')." WHERE identifier='highlight'")) {
			DB::query("UPDATE ".DB::table('common_magic')." SET name='Colorizer', description='You can highlight (change color) a thread or blog title' WHERE identifier='highlight'");
		}
		if(DB::result_first("SELECT name FROM ".DB::table('common_magic')." WHERE identifier='namepost'")) {
			DB::query("UPDATE ".DB::table('common_magic')." SET name='Visitor card', description='You can see a real identity of an anonymous user.' WHERE identifier='namepost'");
		}
		if(DB::result_first("SELECT name FROM ".DB::table('common_magic')." WHERE identifier='anonymouspost'")) {
			DB::query("UPDATE ".DB::table('common_magic')." SET name='Anonymous card', description='Show your name as Anonymous in designated areas.' WHERE identifier='anonymouspost'");
		}

		show_msg('Magics upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Reports upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Application Statistics upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Navigation data upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Group Status Upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Users upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		DB::update('common_credit_rule', array('rulename' => 'Daily login'),"action='daylogin'");
		$count = DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_credit_rule')." WHERE action='portalcomment'");
		if(!$count) {
			DB::query("INSERT INTO ".DB::table('common_credit_rule')." (`rulename`, `action`, `cycletype`, `cycletime`, `rewardnum`, `norepeat`, `extcredits1`, `extcredits2`, `extcredits3`, `extcredits4`, `extcredits5`, `extcredits6`, `extcredits7`, `extcredits8`, `fids`) VALUES ('Comment Article','portalcomment','1','0','40','1','0','1','0','0','0','0','0','0','')");
		}
		$count = DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_credit_rule')." WHERE action='followedcollection'");
		if(!$count) {
			DB::query("INSERT INTO ".DB::table('common_credit_rule')." (`rulename`, `action`, `cycletype`, `cycletime`, `rewardnum`, `norepeat`, `extcredits1`, `extcredits2`, `extcredits3`, `extcredits4`, `extcredits5`, `extcredits6`, `extcredits7`, `extcredits8`, `fids`) VALUES ('Collection followed rule (Taobao album subscription)','followedcollection','1','0','3','0','0','1','0','0','0','0','0','0','')");
		}

		show_msg('Points Rules upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Custom BBCode permissions upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		DB::query("REPLACE INTO ".DB::table('common_smiley')." (typeid, displayorder, type, code, url) VALUES ('4','19','stamp','Edited','010.gif')");
		}
		if(!DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_smiley')." WHERE url='010.small.gif'")) {
		DB::query("REPLACE INTO ".DB::table('common_smiley')." (typeid, displayorder, type, code, url) VALUES ('0','18','stamplist','Edited','010.small.gif')");
		}
		if(!DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_smiley')." WHERE url='011.small.gif'")) {
		DB::query("REPLACE INTO ".DB::table('common_smiley')." (typeid, displayorder, type, code, url) VALUES ('0','20','stamplist','1st post','011.small.gif')");
			$setnewbie = true;
		}
		require_once libfile('function/cache');
		updatecache('stamps');
		...
		show_msg('Thread Stamps upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Permission icons update is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Block Permissions upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Portal channel permissions Upgrade Completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Article upgrade completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Article comments upgrade completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Block templates upgrade completed', "$theurl?step=data&op=$nextop");
		...
		foreach($blocks as $bid => $block) {
			...
			$block = block_convert_to_thread($block);
		...
		show_msg('Block scripts upgrade completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('User group permissions upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Group Home upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Domain Settings upgrade is completed', "$theurl?step=data&op=$nextop");

	} elseif($_GET['op'] == 'pm') {
		$nextop = 'allowgetimage';
			DB::query("UPDATE ".DB::table('common_member')." SET newpm='0', newprompt='0'");
		show_msg('New short message status reset completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('View image rights upgrade completed', "$theurl?step=data&op=$nextop");
		...
		if($updateverify) {
			...
			if($t) {
				...
				if($n) {
					show_msg("Real name verification [$n/$t]", "$theurl?step=data&op=verify&i=$n&updateverify=true");
				}
			}
		}
		show_msg('Real name verification data upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		if($needupgrade && $count) {
			...
			if(DB::num_rows($query)) {
				...
				$start += $limit;
				show_msg("Forum attachment table upgrading ... ".($start/$count), "$theurl?step=data&op=forumattach&start=$start");
			}
		...
		show_msg('Forum attachment table upgrade comleted.', "$theurl?step=data&op=$nextop");

	} elseif($_GET['op'] == 'forumstatlog') {
		$nextop = 'moderate';
		DB::query('DELETE FROM '.DB::table('forum_statlog')." WHERE logdate='0000-00-00'");
		show_msg('Forum Group statistics upgrade is completed', "$theurl?step=data&op=$nextop");
		...
		if($needupgraded) {
			...
			if(!file_exists($cachefile)) {
				...
				if($data && @$fp = fopen($cachefile, 'w')) {
					...
					show_msg('Can not process the thread image table, skipped.', "$theurl?step=data&op=$nextop");
				}
			} else {
				$data = @file($cachefile);
				if(!$data) {
			...
			if(!$tids) {
				@unlink($cachefile);
				show_msg('Thread images upgraded', "$theurl?step=data&op=$nextop");
			}
			$insertsql = array();
			...
			$start += $limit;
			show_msg('Thread images upgrading ... ' . $start, "$theurl?step=data&op=threadimage&start=$start");
		} else {
			show_msg('Can not process the thread image table, skipped.', "$theurl?step=data&op=$nextop");
		}
		...
		show_msg('Verification data upgrade completed.', "$theurl?step=data&op=$nextop");

	} elseif($_GET['op'] == 'moderate_update') {
		$nextop = 'founder';

		show_msg('Moderated data conversion is completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Founder Data Upgrade completed', "$theurl?step=data&op=$nextop");
		...
		show_msg('Plug-in language pack data upgraded.', "$theurl?step=data&op=$nextop");
		...
		show_msg('Remind data upgrade finished', "$theurl?step=data&op=$nextop");
		...
		show_msg('User Medal data upgrade is completed', "$theurl?step=data&op=$nextop");

	} elseif($_GET['op'] == 'closeswitch') {
		$nextop = 'end';
		show_msg('Data update is completed', "$theurl?step=data&op=$nextop");

	} else {

		$deletevar = array('app', 'home');// need to delete items in the Config
		...
		if(!file_exists($default_configfile)) {
			exit('config_global_default.php was not found, please reupload this file.');
		} else {
       ...
       $_config['output']['language'] = $lng_code;

		if(save_config_file($configfile, $_config, $default_config, $deletevar)) {
			show_msg('Data processing is completed', "$theurl?step=delete");
		} else {
			show_msg('"config/config_global.php" file is updated, the "config/" directory is not writable, we have saved the updated file to the "data/" directory, please use FTP software to transfer it to the "config/" directory overwriting the existing file.<br /><br /><a href="'.$theurl.'?step=delete">After completion of this operation click here to continue</a>');
		}
	}
...
//---------------------------------------------------------------------
	...
	show_msg('Data Deletion completed, go to the next step', "$theurl?step=style");
	...
	if($deltables) {
		...
		$deltablehtml .= '</table>';
		echo "<p><strong>The following data tables</strong> are unused after comparing with the standard database:<br>decide whether to delete it if required</p> $deltablehtml";
	}
			...
			foreach ($cols as $coltype => $col) {
				if (is_array($col)) {
					foreach ($col as $index => $indexvalue) {
						$delcolumnhtml .= "<tr><td><input type=\"checkbox\" name=\"delcols[$tablename][$coltype][$index]\" value=\"1\"></td><td>{$config['tablepre']}$tablename</td><td>Index ($coltype) $index $indexvalue</td></tr>";
					}
				} else {
					$delcolumnhtml .= "<tr><td><input type=\"checkbox\" name=\"delcols[$tablename][$col]\" value=\"1\"></td><td>{$config['tablepre']}$tablename</td><td>Field $col</td></tr>";
				}
			}
		}
		$delcolumnhtml .= '</table>';

		echo "<p><strong>The following fields</strong> are unused after comparing with the standard database:<br>decide whether to delete it if required.</p> $delcolumnhtml";
	}

	if(empty($deltables) && empty($delcolumns)) {
		echo "<p>Compared with the standard database, there is no unused data tables and fields need to be deleted</p><a href=\"$theurl?step=style\">Click for go to the next step</a></p>";
	} else {
		echo "<p><input type=\"submit\" name=\"delsubmit\" value=\"Submit for delete\"></p><p>You can also ignore the extra tables and fields<br><a href=\"$theurl?step=style".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">Go to the next step directly</a></p>";
	}
	echo '</form>';
...
//---------------------------------------------------------------------
	...
	if(empty($_GET['confirm'])) {
		show_msg("Please confirm whether you want to restore the default style?<br /><br />
			<a href=\"$theurl?step=style&confirm=yes".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ Yes ]</a>&nbsp;&nbsp;<a href=\"$theurl?step=translate".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ No ]</a>", '');
	}
	...
	show_msg('The default style was restored, go to the next step', "$theurl?step=district");




//==================================================================================
} elseif ($_GET['step'] == 'district') {

	if(empty($_GET['confirm'])) {
		show_msg("Please confirm whether you want to translate the location (country/region) data?<br><br>
			<a href=\"$theurl?step=district&confirm=yes".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ Yes ]</a>&nbsp;&nbsp;
			<a href=\"$theurl?step=translate".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ No ]</a>", '');
	}

	//-- -------------------------------------------------------------------
	show_header();

	echo '<div id="finalmsg">Translating the location data, wait please...</div>';

	//-- -------------------------------------------------------------------

	$sql = 'TRUNCATE TABLE '.DB::table('common_district');

	echo "<p>1) ". htmlspecialchars($sql) ."</p>";
	...
	echo "<p>2) ADD THE COUNTRY/REGION CODE</p>";

	$sql = "SHOW COLUMNS FROM ".DB::table('common_district')." LIKE 'code'";

	echo "<p>". htmlspecialchars($sql) ."</p>";

	if(!$row = DB::fetch_first($sql)) {
		$sql = "ALTER TABLE ".DB::table('common_district')." ADD `code` VARCHAR(64) NOT NULL";
		echo "<p>". htmlspecialchars($sql) ."</p>";
		DB::query($sql);
		flush();
	}
	...
	echo "<p>3) INSERT INTO `pre_common_district` TRANSLATED.</p>";

	for($i = 1; $i <= 3; $i++) {
		$sqlfile = THIS_PATH.'/data/common_district_'.$i.'.sql';
	...
	echo "<p>4) INSERT COUNTRIES INTO `pre_common_district`</p>";

	$sqlfile = THIS_PATH.'/data/common_district_4.sql';
	...
	echo "<p>5) INSERT REGIONS INTO `pre_common_district`</p>";

	$sqlfile = THIS_PATH.'/data/common_district_5.sql';
...
//	show_msg('The default style was restored, go to the next step', "$theurl?step=translate");

	echo "<p><br><br><b>Location data was translated.</b></p>
              <a href=\"$theurl?step=translate\">Click for go to the next step</a></p>";
...
//==================================================================================
} elseif ($_GET['step'] == 'translate') {

	if(empty($_GET['confirm'])) {
		show_msg("Do you want to translate profile fields and settings?<br /><br />
			<a href=\"$theurl?step=translate&confirm=yes".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ Yes ]</a>&nbsp;&nbsp;
                        <a href=\"$theurl?step=cache".($_GET['from'] ? '&from='.rawurlencode($_GET['from']).'&frommd5='.rawurlencode($_GET['frommd5']) : '')."\">[ No ]</a>", '');
	}
	...
	echo "<p>6) Add the 'image' column into 'pre_common_plugin'</p>";

	$sql = "SHOW COLUMNS FROM ".DB::table('common_plugin')." LIKE 'image'";

	echo "<p>". htmlspecialchars($sql) ."</p>";

	if(!$row = DB::fetch_first($sql)) {
		$sql = "ALTER TABLE ".DB::table('common_plugin')." ADD `image` varchar(255) NOT NULL DEFAULT ''";
	...
	echo "<p>7) TRANSLATE other database tables to $lng</p>\n";

	$sqlfile = THIS_PATH.'/data/translate.sql';
	...
	echo "<p>8) Translating is finished!</p>";

	echo '<p>
		<a href="'.$theurl.'?step=cache">Go to the next step</a>
		</p>';
...
//---------------------------------------------------------------------
	...
	if($_GET['from']) {
		show_msg('<div id="finalmsg">Updating the cache, please wait...</div>
			<iframe src="../misc.php?mod=initsys" style="display:none;"
			onload="window.location.href=\''.$_GET['from'].'\'"></iframe>');
	} else {
		show_msg('<div id="finalmsg">Updating the cache, please wait...</div>
			<iframe src="../misc.php?mod=initsys" style="display:none;"
			onload="final();"></iframe>');
...
function show_msg($message, $url_forward='', $time = 1, $noexit = 0, $notice = '') {
	...
	if($url_forward) {
		...
		$message = "<a href=\"$url_forward\">$message (Jump to ...)</a><br>$notice<script>setTimeout(\"window.location.href ='$url_forward';\", $time);</script>";
	}
...
<head>
  ...
  <title> Database upgrade process </title>
  <style type="text/css">
  ...
  <script>
    function final(){
	d=document.getElementById("finalmsg");
	d.innerHTML = "Congratulations!<br> \
		The database structure is updated!<br> \
		The database data is translated!<br> \
		For data security, please delete the folder \
		&quot;/{$curscript}/&quot; from your site!<br><br> \
		<center><a href=../admin.php>Go to your Admin Control Panel</a></center>";
    }
  </script>
</head>
<body>
  <div class="bodydiv">
    <center>
      <h1>Convert Discuz!X to {$lng}</h1>
      <div style="width:90%;margin:0 auto;">
	<table id="menu">
	  <tr>
	    <td{$nowarr[start]}>Start</td>
	    <td{$nowarr[sql]}>Update the Database</td>
	    <td{$nowarr[data]}>Update the Data</td>
	    <td{$nowarr[style]}>Reset Style</td>
            <td{$nowarr[delete]}>Delete unused data</td>
	    <td{$nowarr[translate]}>Translate</td>
	    <td{$nowarr[cache]}>Clean Cache</td>
	  </tr>
	</table>
	<br>
END;
...
function show_footer() {
	...
	print <<<END
      ...
      <div id="footer">&copy; 2008-{$y} Valery Votintsev <a href="http://discuz.ml/">Discuz.ML</a></div>
    </center>
  </div>
  <br>
</body>
...
//---------------------------------------------------------------------
function block_convert_to_thread($block){
	...
	$block['styleid'] = 0;
	$block['blockstyle'] = block_style_convert_to_thread($block['blockstyle'], $block['blockclass']);
	return $block;
}

//---------------------------------------------------------------------
function block_style_convert_to_thread($style, $blockclass) {
