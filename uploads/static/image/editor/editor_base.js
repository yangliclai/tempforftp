...
/*vot*/	if(window.confirm('Are you sure you want to restore to last saved?') {
		var data = loadUserdata('home');
		if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
/*vot*/			parent.showDialog('No data can be recoverd!');
			return;
		}
...
function format(type, para){
	...
	if(!gIsIE){
		switch(type){
			case "Cut":
/*vot*/				sAlert = 'Your browser security settings does not permit the editor to automatically execute the Cutting operation. Use the keyboard shortcut (Ctrl X) to complete this operation.';
				break;
			case "Copy":
/*vot*/				sAlert = 'Your browser security settings does not permit the editor to automatically execute the Copy operation. Use the keyboard shortcut (Ctrl C) to complete this operation.';
				break;
			case "Paste":
/*vot*/				sAlert = 'Your browser security settings does not permit the editor to automatically execute the Paste operation. Use the keyboard shortcut (Ctrl V) to complete this operation.';
				break;
		}
	}
	if(sAlert != ""){
...
/*vot*/		alert('Can not find the Graffiti initialization data');
	}
}
...
function fDisplayElement(element,displayValue) {
	if(gIEVer<=5.01 && gIsIE){
/*vot*/		alert('Supported only in IE 5.01 or later');
		return;
	}
	fHideMenu();
...
/*vot*/		dv.innerHTML = 'Edit Raw Text';
...
function changeEditType(flag, ev){
	...
	try{
		...
		if(flag){
			...
			ev = ev || event;
			if(ev){
/*vot*/				if(window.confirm('Converting to the plain text will lose some formatting!\nAre you sure you want to continue?')){
...
/*vot*/		ele.innerHTML = flag ? 'Restore size' : 'Full Screen';
