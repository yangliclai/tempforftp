...
function previewFormSubmit() {
	...
	if (!selectedTopicId) {
/*vot*/		alert('Select at least one topic for push to Articles');
		return false;
	}
	if (selectedNormalIds.length < 1) {
/*vot*/		alert('Select at least one item for push to Articles');
		return false;
	}
...
/*vot*/	cloudj('#search_result').html('<tr><td colspan="3">Loading...</td></tr>');
...
/*vot*/		alert('Push Post number has reached five, in the right to cancel a number and try again.');
		return false;
	}
...
/*vot*/		firstThreadLi.html('<div class="tips">Click on the left <img src="static/image/admincp/cloud/qun_op_list.png" align="absmiddle" /> Will push the information to the list</div>');
		firstThreadLi.show();
	}
}
...
/*vot*/	cloudj('#uploadImageResult').text('Upload image, please wait...');
...
function qqgroupajaxpost(formid, showid, waitid, showidclass, submitbtn, recall) {
	...
	var handleResult = function() {
		...
		if (!s) {
/*vot*/			s = 'Internal Error, can not display this content';
		}
