...
/*vot*/					aj.resultHandle('<a href="' + aj.targetUrl + '" target="_blank" style="color:red">Internal Error, can not display this content</a>' , aj);
				} else {
...
/*vot*/	var waiting = waiting ? waiting : 'Loading ...';
...
/*vot*/			$('css_' + cssname).href = csspath + STYLEID + '_' + cssname + '.css?' + VERHASH;
		}
		CSSLOADED[cssname] = 1;
	}
}
...
/*vot*/	confirmtxtdefault = 'Submit';
	...
	if(locationtime) {
/*vot*/		leftmsg = locationtime + ' seconds before the page redirect';
		showDialogST = setTimeout(closefunc, locationtime * 1000);
		showconfirm = 0;
	}
	confirmtxt = confirmtxt ? confirmtxt : confirmtxtdefault;
/*vot*/	canceltxt = canceltxt ? canceltxt : 'Cancel';
...
/*vot*/	s += t ? t : 'Reminder';
/*vot*/	s += '</em><span><a href="javascript:;" id="fwin_dialog_close" class="flbc" onclick="hideMenu(\'' + menuid + '\', \'dialog\')" title="Close">' /*+'Close'*/ +'</a></span></h3>';
	if(mode == 'info') {
		s += msg ? msg : '';
	} else {
...
/*vot*/			loadingst = setTimeout(function() {showDialog('', 'info', '<img src="' + IMGDIR + '/loading.gif"> Loading ...')}, 500);
		}
	};
...
function showError(msg) {
	...
	msg = msg.replace(p, '');
	if(msg !== '') {
/*vot*/		showDialog(msg, 'alert', 'Error Message', null, true, null, '', '', '', 3);
	}
}
...
/*vot*/		if(searchtxt.value == 'Enter search words') {
			searchtxt.value = '';
		}
/*vot*/		searchtxt.placeholder = 'Enter search words';
	} else {
...
/*vot*/			if(searchtxt.value == 'Enter search words') {
		...
		searchtxt.onblur = function () {
			if(searchtxt.value == '' ) {
/*vot*/				searchtxt.value = 'Enter search words';
				searchtxt.className = tclass + ' xg1';
			}
		};
	}
...
/*vot*/	if(obj.value == 'Enter search words') {
		obj.value = '';
	}
...
/*vot*/		document.title = (NOTICETITLE['State'] ? '[　　　]' : '[New]') + NOTICETITLE['oldTitle'];
...
/*vot*/			showDialog('Press Ctrl + D keys for add to Favorites', 'notice');
		}
	}
}
...
/*vot*/		showDialog('For non-IE browser please set Homepage manually', 'notice');
		doane();
	}
}
...
/*vot*/			dom.title = 'Disable blind experience';
		} else {
			saveUserdata('is_blindman', '1');
/*vot*/			dom.title = 'Enable blind experience';
		}
	}
}
