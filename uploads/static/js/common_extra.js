...
/*vot*/	setCopy(BROWSER.ie ? obj.innerText.replace(/\r\n\r\n/g, '\r\n') : obj.textContent, 'The code was copied to clipboard');
}
...
/*vot*/			showDialog('<div class="c"><div style="width: 200px; text-align: center;">Copy failed, please select &quot;Allow access&quot;</div></div>', 'alert');
		}
	} else {
/*vot*/		var msg = '<div class="c"><div style="width: 200px; text-align: center; text-decoration:underline;">Copy to clipboard</div>' +
...
function _showselect(obj, inpid, t, rettype) {
	...
	if(!obj.id) {
		...
		s = '';
		if(!t) {
/*vot*/			s += showselect_row(inpid, '1 Day', 1, 0, rettype);
/*vot*/			s += showselect_row(inpid, '1 Week', 7, 0, rettype);
/*vot*/			s += showselect_row(inpid, '1 Month', 30, 0, rettype);
/*vot*/			s += showselect_row(inpid, '3 Months', 90, 0, rettype);
/*vot*/			s += showselect_row(inpid, 'Custom', -2);
		} else {
			if($(t)) {
...
/*vot*/				s += showselect_row(inpid, 'Custom', -1);
			} else {
/*vot*/				s += '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = \'0\'">Permanent</a>';
/*vot*/				s += showselect_row(inpid, '7 Days', 7, 1, rettype);
/*vot*/				s += showselect_row(inpid, '14 Days', 14, 1, rettype);
/*vot*/				s += showselect_row(inpid, '1 Month', 30, 1, rettype);
/*vot*/				s += showselect_row(inpid, '3 Months', 90, 1, rettype);
/*vot*/				s += showselect_row(inpid, '6 Months', 182, 1, rettype);
/*vot*/				s += showselect_row(inpid, '1 Year', 365, 1, rettype);
/*vot*/				s += showselect_row(inpid, 'Custom', -1);
			}
		}
		$(div.id).innerHTML = s;
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	if(!menu && !pn) {
		...
		menu.id = menuid;
		if(cover) {
/*vot*/			menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none"><p><span class="y"><a id="' + menuid + '_imglink" class="imglink" target="_blank" title="Open in a new window">Open in a new window</a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="Actual Size">Actual Size</a>' +
/*vot*/				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="Close">Close</a></span> Use mouse wheel to zoom in/out the image</p>' +
				'<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		} else {
/*vot*/			menu.innerHTML = '<div class="popupmenu_popup" id="' + menuid + '_zoomlayer" style="width:auto"><span class="right y"><a href="javascript:;" onclick="hideMenu()" class="flbc" style="width:20px;margin:0 0 2px 0">Close</a></span> Use mouse wheel to zoom in/out the image<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		}
...
/*vot*/				picpage += ' <div id="zimg_prev" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'0 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'0 -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_prev"><strong>Prev.</strong></div> ';
				paid = authorcurrent < authorlength - 1 ? authorimgs[authorcurrent + 1] : authorimgs[0];
/*vot*/				picpage += ' <div id="zimg_next" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'100% 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'100% -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_next"><strong>Next</strong></div> ';
			}
			if(picpage) {
...
/*vot*/		var colortexts = ['Black', 'Sienna', 'Dark Olive Green', 'Dark Green', 'Dark Gray Blue', 'Navy', 'Indigo', 'Dark Green', 'Dark Red', 'Dark Orange', 'Olive', 'Green', 'Teal', 'Blue', 'Limestone', 'Dark Gray', 'Red', 'Brown Sand', 'Yellow Green', 'Sea Green', 'Green emerald', 'Royal Blue', 'Purple', 'Gray', 'Red Purple', 'Orange', 'Yellow', 'Acid Orange', 'Blue Green', 'Deep Sky Blue', 'Dark Purple', 'Silver', 'Pink', 'Light Yellow', 'Lemon Silk', 'Cang Green', 'Cang gem Green', 'Bright blue', 'Plum color', 'White'];
		var str = '';
		for(var i = 0; i < 40; i++) {
...
/*vot*/	var strs = ['Wide screen', 'Narrow screen'];
	if($('css_widthauto')) {
...
/*vot*/		menu.innerHTML = '<div class="p_opt"><img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" /> Loading ...</div>';
		$('append_parent').appendChild(menu);
...
/*vot*/		var msg = '1. Click "<a href="javascript:;" class="xi2 xw1" ';
		msg += 'onclick="this.href = \'forum.php?mod=misc&action=shortcut\';this.click();saveUserdata(\'setshortcut\', 1);"';
/*vot*/		msg += '>Download desktop shortcut</a>", and after the download is completed, you can move the file to your desktop<br />';
/*vot*/		msg += '2. Click "<a href="forum.php?mod=misc&action=shortcut&type=ico" class="xi2 xw1">';
/*vot*/		msg += 'Download icon file</a>", and after the download is completed, right-click on the desktop shortcut file -&gt; Properties -&gt; Change Icon, and select the downloaded ICO icon';
/*vot*/		showDialog(msg, 'notice', 'Adding a desktop shortcut');
	};
