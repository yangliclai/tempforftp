...
/*vot*/	return confirm('Are you sure you want to remove this thread from hot threads?');
}
...
/*vot*/			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">There are new thread replies, click for View', 'colspan': colspan }}};
...
function showtime() {
	for(i=0; i<=DTimers.length; i++) {
		if(DItemIDs[i]) {
			if(DTimers[i] == 0) {
/*vot*/				$(DItemIDs[i]).innerHTML = 'Finished';
				DItemIDs[i] = '';
				continue;
			}
			var timestr = '';
			...
			if(timer_day > 0) {
/*vot*/				timestr += timer_day + ' days';
			}
			if(timer_hour > 0) {
/*vot*/				timestr += timer_hour + ' hours';
			}
			if(timer_minute > 0) {
/*vot*/				timestr += timer_minute + ' minutes';
			}
			if(timer_second > 0) {
/*vot*/				timestr += timer_second + ' sec.';
			}
