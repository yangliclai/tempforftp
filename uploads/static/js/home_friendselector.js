		...
		selectUserName:function(userName) {
			...
			if(userName != '') {
				...
				if(uid && typeof this.selectUser[uid] == 'undefined' || uid === 0 && typeof this.selectUser[userName] == 'undefined') {
					...
					this.selectNumber++;
/*vot*/					spanObj.innerHTML= '<a href="javascript:;" class="x" onclick="'+this.handleKey+'.delSelUser(\''+(spanObj.id)+'\');">Delete</a><em class="z" title="' + userName + '">' + userName + '</em><input type="hidden" name="users[]" value="'+userName+'" uid="uid'+uid+'" />';
					this.handleObj.parentNode.insertBefore(spanObj, this.handleObj);
					this.showObj.style.display = 'none';
				} else {
/*vot*/					alert('Already exists: '+userName);
				}
			}
		},
...
/*vot*/			            alert('You can select up to '+this.maxSelectNumber+' users');
			            return false;
			        }
					this.selectUser[uid] = this.dataSource[uid];
					this.selectNumber++;
