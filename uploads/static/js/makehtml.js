...
/*vot*/				obj.innerHTML = '<a href="'+ret['path']+'" target="_blank">'+title+' generated successfully</a>';
			}
			if(ret['nexturl']) {
				if(obj) {
					obj.style.color = 'green';
/*vot*/					obj.innerHTML = 'Generate '+title+(Math.round((ret['current']/ret['count'])*100))+'%';
				}
			...
			if(obj) {
				obj.style.color = 'red';
/*vot*/				obj.innerHTML = title+' generation failed';
			}
		}
	});
...
function make_html_batch(url, ids, callback, dom, single) {
	...
	if(this.single) {
		...
		progress_bar(cent);
/*vot*/		child.innerHTML = 'Start generating of '+this.dom.getAttribute('mktitle');
		this.dom.innerHTML = '';
		this.dom.appendChild(child);
		this.make(id, child);
		this.child = child;
		var child2 = document.createElement('div');
/*vot*/		child2.innerHTML = '<a href="javascript:void(0);" id="mk_goon">If your browser does not respond, Click to continue...</a>';
...
/*vot*/					makehtml_error.innerHTML = 'Error Message';
...
/*vot*/				child.innerHTML = '<div class="mk_msg">'+'<a href="'+data['path']+'" target="_blank">'+obj.dom.getAttribute('mktitle')+'</a> generation is completed</div>';
				if(obj.callback) {
				...
				progress_bar(cent);
/*vot*/				var str = 'Total need to generate '+obj.count+' items, '+obj.dom.getAttribute('mktitle')+' files, Successfully generated '+obj.makedcount+' items, ';
				if(cent != 100) {
/*vot*/					child.innerHTML = str+' generated first '+current+' items, has been completed '+cent+'%';
				} else {
					child.style.color = 'blue';
/*vot*/					child.innerHTML = str+obj.dom.getAttribute('mktitle')+' generation is completed';
				}
				if(id) {
					obj.make(id, child);
