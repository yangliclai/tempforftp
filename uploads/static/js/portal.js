...
/*vot*/	var msg = itemtype==1 ? 'Are you sure to delete this data?' : 'Are you sure to ignore this data?';
	if(confirm(msg)) {
...
/*vot*/		alert('Please select a block!');
		return false;
	}
}
...
/*vot*/			back.innerHTML = '<em id="recommendback" onclick="recommenditem_back()" class="cur1">&nbsp;&nbsp;&laquo; Back</em>';
...
/*vot*/		$('a_setting_show').innerHTML = 'Show settings';
	} else {
		$('tbody_setting').style.display = '';
/*vot*/		$('a_setting_show').innerHTML = 'Hide settings';
	}
	doane();
}
function switchSetting() {
	...
	if(checked) {
		$('tbody_setting').style.display = 'none';
/*vot*/		$('a_setting_show').innerHTML = 'Expand setting';
	} else {
		$('tbody_setting').style.display = '';
/*vot*/		$('a_setting_show').innerHTML = 'Hide setting';
	}
}

function checkblockname(form) {
	if(!(trim(form.name.value) > '')) {
/*vot*/		showDialog('Block name can not be empty', 'error', null, function(){form.name.focus();});
		return false;
	}
	if(form.summary && form.summary.value) {
		...
		if(tag) {
			showBlockSummary();
			form.summary.focus();
/*vot*/			showDialog('Custom content error! HTML code: HTML Code: '+tag+' - Tags does not match', 'error', null, function(){form.summary.select();});
			return false;
		}
	}
...
/*vot*/					showDialog('HTML Code: '+tag+' - Tags does not match', 'error', null, fn, true, fn);
					return false;
				}
			}
		}
...
function blockconver(ele,bid) {
	if(ele && bid) {
/*vot*/		if(confirm('Are you sure you want to convert the block from type '+ele.options[0].innerHTML+' to '+ele.options[ele.selectedIndex].innerHTML)) {
...
function check_itemdata_lentgh(form) {
	if(form.title && (!strLenCalc(form.title, "titlechk", form.title.getAttribute('_maxlength')) || !form.title.value)) {
		form.title.focus();
/*vot*/		showDialog('The title is incorrect', 'error', null, function(){form.title.select();});
		return false;
	}
	if(form.summary && !strLenCalc(form.summary, "summarychk", form.summary.getAttribute('_maxlength'))) {
		form.summary.focus();
/*vot*/		showDialog('Summary length is incorrect', 'error', null, function(){form.summary.select();});
		return false;
	}
	return true;
}