...
/*vot*/				buttons[i].innerHTML = !editorisfull ? 'Full Screen' : 'Restore size';
/*vot*/				buttons[i].onmouseover = function(e) {setEditorTip(editorisfull ? 'Resume editor size' : 'Full Screen Editor');};
				buttons[i].onclick = function(e) {editorfull();doane();}
			} else if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'simple') {
/*vot*/				buttons[i].innerHTML = !simplodemode ? 'General Mode' : 'Advanced Mode';
...
/*vot*/			showDialog('Your browser does not support this feature, please upgrade your browser version', 'notice', 'Tips');
		};
...
function savedataTime() {
	if(!autosave) {
/*vot*/		$(editorid + '_svdsecond').innerHTML = '<a title="Click here for enable the auto-saving" href="javascript:;" onclick="setAutosave()">Enable the auto-saving</a> ';
		return;
	}
	if(!savedatac) {
		savedatac = savedataInterval;
		...
		h = h < 10 ? '0' + h : h;
		m = m < 10 ? '0' + m : m;
/*vot*/		setEditorTip('Data saved at ' + h + ':' + m + ' ');
	}
/*vot*/	$(editorid + '_svdsecond').innerHTML = '<a title="Disable the auto-saving" href="javascript:;" onclick="setAutosave()">' + savedatac + ' sec. before autosave</a> ';
	savedatac -= 10;
}

function setAutosave() {
	autosave = !autosave;
/*vot*/	setEditorTip(autosave ? 'Data auto-saving enabled' : 'Data auto-saving disabled');
...
/*vot*/	$(editorid + '_fullswitcher').innerHTML = editorisfull ? 'Back' : 'Full Screen';
	initesbar();
}
...
function editorsimple() {
	if($(editorid + '_body').className == 'edt') {
		v = 'none';
/*vot*/		$(editorid + '_simple').innerHTML = 'Advanced Mode';
		...
		v = '';
/*vot*/		$(editorid + '_simple').innerHTML = 'General Mode';
...
/*vot*/	showDialog('Current Length: ' + mb_strlen(message) + ' characters, ' + (postmaxchars != 0 ? 'System limit: ' + postminchars + ' ~ ' + postmaxchars + ' characters.' : ''), 'notice', 'Length Count');
}
...
/*vot*/			while(listvalue = prompt('Enter the item list.\r\nLeave blank, or click Cancel.', '')) {
				if(BROWSER.opera > 8) {
					listvalue = '\n' + '[*]' + listvalue;
		...
		loadData();
/*vot*/		setEditorTip('The Data was restored');
	} else if(cmd == 'svd') {
		saveData();
/*vot*/		setEditorTip('Data saved');
	} else if(cmd == 'chck') {
		checklength(editorform);
	} else if(cmd == 'tpr') {
/*vot*/		if(confirm('Are you sure to clear all the contents?')) {
			clearContent();
		}
	} else if(cmd == 'downremoteimg') {
/*vot*/		showDialog('<div id="remotedowninfo"><p class="mbn">Downloading remote attachment, please wait ...</p><p><img src="' + STATICURL + 'image/common/uploading.gif" alt="" /></p></div>', 'notice', '', null, 1);
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'url':
/*vot*/				str = 'Enter the link URL:<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" />'+
/*vot*/					(selection ? '' : '<br />Enter the link text:<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="px" />');
				break;
			case 'forecolor':
			...
			case 'postbg':
				...
				break;
			case 'password':
/*vot*/				str = '<p class="pbn">Please enter the post password: <input type="text" id="' + ctrlid + '_param_1" size="10" value="" class="px" /></p>';
				break;
			case 'code':
				if(wysiwyg) {
...
/*vot*/				var lang = {'quote' : 'Insert the Quote', 'code' : 'Insert the Code', 'hide' : 'Hide content', 'free' : 'If you did not set a post price, the entered information can be seen free of charge before purchasing the content,'};
				str += lang[tag] + ':<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>' +
/*vot*/					(tag == 'hide' ? '<br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="pc" checked="checked" />Show only when the user reply to this thread </label><br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="pc" />Show only when the user points is more than </label> <input type="text" size="3" id="' + ctrlid + '_param_2" class="px pxs" /> When to show<br /><br /><label>Valid for (days):</label> <input type="text" size="3" id="' + ctrlid + '_param_3" class="px pxs" />' + '<br />Distance from the posting date is greater than the number of days when the label automatically expire' : '');
				break;
			case 'tbl':
/*vot*/				str = '<p class="pbn">Table Rows: <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="px" /> &nbsp; Table Columns: <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="px" /></p><p class="pbn">Table Width: <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="px" /> &nbsp; Background Color: <input type="text" id="' + ctrlid + '_param_4" size="2" class="px" onclick="showColorBox(this.id, 2)" /></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'Tips\', null, 0)"><img id="tbltips" title="Tips" class="vm" src="' + IMGDIR + '/info_small.gif"> Quick write table tips</p>';
/*vot*/				str += '<div id="tbltips_msg" style="display: none">&quot;[tr=color]&quot; Define the row background color<br />&quot;[td=Width]&quot; Define the column width<br />&quot;[td=Column_Span,Row_Span,Width]&quot; Define the Row/Column Span and Width<br /><br />Fast writing table example: <div class=\'xs0\' style=\'margin:0 5px\'>[table]<br />Name:|Discuz!<br />Version:|X1.5<br />[/table]</div>Use &quot;|&quot; to separate rows, if there is the &quot;|&quot; character in the data, replace it with &quot;\\|&quot;, separate rows with &quot;\\n&quot;.</div>';
				break;
			case 'aud':
/*vot*/				str = '<p class="pbn">Input URL of music file:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">Supported wma, mp3, ra, rm, and other music formats<br />Example: http://server/audio.wma</p>';
				break;
			case 'vid':
/*vot*/				str = '<p class="pbn">Input URL of video file:</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">Width: <input id="' + ctrlid + '_param_2" size="5" value="500" class="px" /> &nbsp; Height: <input id="' + ctrlid + '_param_3" size="5" value="375" class="px" /></p><p class="xg2 pbn">Supported for Youku, Potatoes, 56, 6, Cool video and other video services<br />Supported video formats: wmv avi rmvb mov swf flv.<br /> Example: http://server/movie.wmv</p>';
				break;
			case 'fls':
/*vot*/				str = '<p class="pbn">Please input URL of Flash file:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">Width: <input id="' + ctrlid + '_param_2" size="5" value="" class="px" /> &nbsp; Height: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /></p><p class="xg2 pbn">Supported Flash formats: swf, flv <br /> Example: http://server/flash.swf</p>';
				break;
			case 'beginning':
/*vot*/				str = '<p class="pbn">Enter the beginning Flash or image animation URL:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p>';
/*vot*/				str += '<p class="pbn">Click on the link URL:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_2" class="px" value="" style="width: 220px;" /></p>';
/*vot*/				str += '<p class="pbn">Width: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /> &nbsp; Height: <input id="' + ctrlid + '_param_4" size="5" value="" class="px" /></p>';
/*vot*/				str += '<p class="pbn">Display seconds: <input id="' + ctrlid + '_param_8" size="5" value="" class="px" /></p>';
/*vot*/				str += '<p class="pbn">Disappear effect after loading: </p><p class="pbn"><input id="' + ctrlid + '_param_7" type="radio" name="effect" checked />None &nbsp; <input id="' + ctrlid + '_param_5" type="radio" name="effect" />Fade &nbsp; <input id="' + ctrlid + '_param_6" type="radio" name="effect" />Explode</p>';
/*vot*/				str += '<p class="xg2 pbn">Supported formats: swf flv jpg gif png<br/>Width range: 400~1024, Height range: 300~640<br/>Example: http://server/flash.swf</p>';
				break;
			case 'pasteword':
/*vot*/				stitle = 'Paste a content from Word';
/*vot*/				str = '<p class="px" style="height:300px"><iframe id="' + ctrlid + '_param_1" frameborder="0" style="width:100%;height:100%" onload="this.contentWindow.document.body.style.width=\'550px\';this.contentWindow.document.body.contentEditable=true;this.contentWindow.document.body.focus();this.onload=null"></iframe></p><p class="xg2 pbn">Please use shortcut (Ctrl + V) to paste the content from Word document</p>';
				menuwidth = 600;
				menupos = '00';
				menutype = 'win';
				break;
			case 'index':
/*vot*/				stitle = 'Create post directory';
/*vot*/				str = '<p class="pbn">[index]<br />\n\
					[#<span class="xi1">Page number</span>]<span class="xi1">Title</span> &nbsp;&nbsp;<span class="xg1">Jump to specified page</span><br />\n\
					<span class="xi1">*</span>[#<span class="xi1">tid,pid</span>]<span class="xi1">Title</span> &nbsp;&nbsp;<span class="xg1">Jump to specified post</span><br />\n\
					[/index]<br />\n\
					<br />\n\
					<span class="xi1">Page number</span> &nbsp;&nbsp;<span class="xg1">Use [page] tag for add a page break</span><br />\n\
					<span class="xi1">tid,pid</span> &nbsp;&nbsp;<span class="xg1">Post TID and PID</span><br />\n\
					<span class="xi1">*</span> &nbsp;&nbsp;<span class="xg1">Add the first line indent</span></p>';
				break;
//vot: googlemap by yongbing@ichangning.com
/*vot*/			case 'googlemap':
/*vot*/				stitle = 'Google Maps';
/*vot*/				str = '<p class="px" style="height:390px"><iframe id="' + ctrlid + '_param_1" frameborder="0" style="width:100%;height:100%" onload="this.contentWindow.document.body.style.width=\'530px\';this.onload=null" src="./misc.php?mod=googlemap"></iframe></p><p class="xg2 pbn">Insert Google Map</p>';
/*vot*/				menuwidth = 600;
/*vot*/				menupos = '00';
/*vot*/				menutype = 'win';
				break;
			default:
				...
				if(!str) {
					...
					for(var i = 1; i <= params; i++) {
						if(i != params || !haveSel) {
/*vot*/						str += (promptlang[i - 1] ? promptlang[i - 1] : 'Please enter the ' + i + '-th parameter:') + '<br /><input type="text" id="' + ctrlid + '_param_' + i + '" style="width: 98%" value="" class="px" />' + (i < params ? '<br />' : '');
						}
					}
				}
				break;
...
/*vot*/				+ '<h3 class="flb"><em>' + stitle + '</em><span><a onclick="hideMenu(\'\', \'win\');return false;" class="flbc" href="javascript:;">Close</a></span></h3><div class="c">' + str + '</div>'
/*vot*/				+ '<p class="o pns"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>Submit</strong></button></p>'
				+ '</td><td class="m_r"></td></tr><tr><td class="b_l"></td><td class="b_c"></td><td class="b_r"></td></tr></table>';
		} else {
/*vot*/			s = '<div class="p_opt cl"><span class="y" style="margin:-10px -10px 0 0"><a onclick="hideMenu();return false;" class="flbc" href="javascript:;">Close</a></span><div>' + str + '</div><div class="pns mtn"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>Submit</strong></button></div></div>';
		}
		menu.innerHTML = s;
