...
drag.extend({
	'getBlocksTimer' : '',
	'blocks' : [],
/*vot*/	'blockDefaultClass' : [{'key':'Choose a Style','value':''},{'key':'No border, no margin','value':'cl_block_bm'},{'key':'Style1','value':'xbs_1'},{'key':'Style2','value':'xbs xbs_2'},{'key':'Style3','value':'xbs xbs_3'},{'key':'Style4','value':'xbs xbs_4'},{'key':'Style5','value':'xbs xbs_5'},{'key':'Style6','value':'xbs xbs_6'},{'key':'Style7','value':'xbs xbs_7'}],
/*vot*/	'frameDefaultClass' : [{'key':'Choose a Style','value':''},{'key':'No border, no margin','value':'cl_frame_bm'},{'key':'No border frame','value':'xfs xfs_nbd'},{'key':'Style1','value':'xfs xfs_1'},{'key':'Style2','value':'xfs xfs_2'},{'key':'Style3','value':'xfs xfs_3'},{'key':'Style4','value':'xfs xfs_4'},{'key':'Style5','value':'xfs xfs_5'}],
	setDefalutMenu : function () {
/*vot*/		this.addMenu('default','Title','drag.openTitleEdit(event)');
/*vot*/		this.addMenu('default','Style','drag.openStyleEdit(event)');
/*vot*/		this.addMenu('default', 'Delete', 'drag.removeBlock(event)');
/*vot*/		this.addMenu('block', 'Attribute', 'drag.openBlockEdit(event)');
/*vot*/		this.addMenu('block', 'Data', 'drag.openBlockEdit(event,"data")');
/*vot*/		this.addMenu('block', 'Update', 'drag.blockForceUpdate(event)');
/*vot*/		this.addMenu('frame', 'Export', 'drag.frameExport(event)');
/*vot*/		this.addMenu('tab', 'Export', 'drag.frameExport(event)');
	},
...
/*vot*/		var repeatarr = [{'key':'Repeat','value':'repeat'},{'key':'No repeat','value':'no-repeat'},{'key':'Repeat Horizontally','value':'repeat-x'},{'key':'Repeat Vertically','value':'repeat-y'}];
/*vot*/		var stylearr = [{'key':'No style','value':'none'},{'key':'Solid Line','value':'solid'},{'key':'Dotted Line','value':'dotted'},{'key':'Dased Line','value':'dashed'}];
		var table = '<table class="tfm">';
/*vot*/		table += '<tr><th>Font</th><td><input type="text" id="fontsize" class="px p_fre vm" value="'+fontsize+'" size="2" />px <input type="text" id="fontcolor" class="px p_fre vm" value="'+fontcolor+'" size="2" />';
		table += getColorPalette(id+'_fontPalette', 'fontcolor' ,fontcolor)+'</td></tr>';
/*vot*/		table += '<tr><th>Link</th><td><input type="text" id="linkfontsize" class="px p_fre vm" value="'+linkfontsize+'" size="2" />px <input type="text" id="linkcolor" class="px p_fre vm" value="'+linkcolor+'" size="2" />';
...
/*vot*/		table += '<tr><th>Border</th><td><ul id="borderul" class="'+ulclass+'">';
/*vot*/		table += '<li><label>Top</label><select class="ps vm" id="bdtwidth" ><option value="">Size</option>'+this.getOption(widtharr,bdtwidth)+'</select>';
/*vot*/		table += ' <select class="ps vm" id="bdtstyle" ><option value="">Style</option>'+this.getOption(stylearr,bdtstyle)+'</select>';
/*vot*/		table += ' Color <input type="text" id="bdtcolor" class="px p_fre vm" value="'+bdtcolor+'" size="7" />';
		table += getColorPalette(id+'_bdtPalette', 'bdtcolor' ,bdtcolor)+'</li>';

/*vot*/		table += '<li class="bordera mtn"><label>Right</label><select class="ps vm" id="bdrwidth" ><option value="">Size</option>'+this.getOption(widtharr,bdrwidth)+'</select>';
/*vot*/		table += ' <select class="ps vm" id="bdrstyle" ><option value="">Style</option>'+this.getOption(stylearr,bdrstyle)+'</select>';
/*vot*/		table += ' Color <input type="text" id="bdrcolor" class="px p_fre vm" value="'+bdrcolor+'" size="7" />';
		table += getColorPalette(id+'_bdrPalette', 'bdrcolor' ,bdrcolor)+'</li>';

/*vot*/		table += '<li class="bordera mtn"><label>Bottom</label><select class="ps vm" id="bdbwidth" ><option value="">Size</option>'+this.getOption(widtharr,bdbwidth)+'</select>';
/*vot*/		table += ' <select class="ps vm" id="bdbstyle" ><option value="">Style</option>'+this.getOption(stylearr,bdbstyle)+'</select>';
/*vot*/		table += ' Color <input type="text" id="bdbcolor" class="px p_fre vm" value="'+bdbcolor+'" size="7" />';
		table += getColorPalette(id+'_bdbPalette', 'bdbcolor' ,bdbcolor)+'</li>';

/*vot*/		table += '<li class="bordera mtn"><label>Left</label><select class="ps vm" id="bdlwidth" ><option value="">Size</option>'+this.getOption(widtharr,bdlwidth)+'</select>';
/*vot*/		table += ' <select class="ps vm" id="bdlstyle" ><option value="">Style</option>'+this.getOption(stylearr,bdlstyle)+'</select>';
/*vot*/		table += ' Color <input type="text" id="bdlcolor" class="px p_fre vm" value="'+bdlcolor+'" size="7" />';
		table += getColorPalette(id+'_bdlPalette', 'bdlcolor' ,bdlcolor)+'</li>';
/*vot*/		table += '</ul><p class="ptm"><label><input id="borderop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'borderul\').className = $(\'borderul\').className == \'borderul\' ? \'borderula\' : \'borderul\'"> Separate Config</label></p></td></tr>';
...
/*vot*/		table += '<tr><th>Margin</th><td><div id="margindiv" class="'+ulclass+'"><span><label>Top</label> <input type="text" id="margint" class="px p_fre vm" value="'+margint+'" size="1"/>px </span>';
/*vot*/		table += '<span class="bordera"><label>Right</label><input type="text" id="marginr" class="px p_fre vm" value="'+marginr+'" size="1" />px </span>';
/*vot*/		table += '<span class="bordera"><label>Bottom</label><input type="text" id="marginb" class="px p_fre vm" value="'+marginb+'" size="1" />px </span>';
/*vot*/		table += '<span class="bordera"><label>Left</label><input type="text" id="marginl" class="px p_fre vm" value="'+marginl+'" size="1" />px</span>';
/*vot*/		table += '</div><p class="ptm"><label><input id="marginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'margindiv\').className = $(\'margindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'"> Separate Config</label></p></td></tr>';
...
/*vot*/			table += '<tr><th>Padding</th><td><div id="cmargindiv" class="'+ulclass+'"><span><label>Top</label> <input class="px p_fre" id="cmargint" value="'+cmargint+'" size="1" />px </span>';
/*vot*/			table += '<span class="bordera"><label>Right</label><input class="px p_fre" id="cmarginr" value="'+cmarginr+'" size="1" />px </span>';
/*vot*/			table += '<span class="bordera"><label>Bottom</label><input class="px p_fre" id="cmarginb" value="'+cmarginb+'" size="1" />px </span>';
/*vot*/			table += '<span class="bordera"><label>Left</label><input class="px p_fre" id="cmarginl" value="'+cmarginl+'" size="1" />px </span>';
/*vot*/			table += '</div><p class="ptm"><label><input id="cmarginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'cmargindiv\').className = $(\'cmargindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'"> Separate Config</label></p></td></tr>';
		}
/*vot*/		table += '<tr><th>Background Color</th><td><input type="text" id="bgcolor" class="px p_fre vm" value="'+bgcolor+'" size="4" />';
		table += getColorPalette(id+'_bgcPalette', 'bgcolor' ,bgcolor)+'</td></tr>';
/*vot*/		table += '<tr><th>Background Image</th><td><input type="text" id="bgimage" class="px p_fre vm" value="'+bgimage+'" size="25" /> <select class="ps vm" id="bgrepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		var classarr = objType == 1 ? this.blockDefaultClass : this.frameDefaultClass;
/*vot*/		table += '<tr><th>Designated Class</th><td><input type="text" id="diyClassName" class="px p_fre" value="'+diyClassName+'" size="8" /> <select class="ps vm" id="bgrepeat" onchange="$(\'diyClassName\').value=this.value;" >'+this.getOption(classarr, diyClassName)+'</select></td></tr>';
		table += '</table>';

/*vot*/		var wname = objType ? 'Block' : 'Frame';
		html = '<div class="c diywin" style="width:450px;position:relative;">'+table+'</div>';
/*vot*/		var h = '<h3 class="flb"><em>Edit '+wname+' Style</em><span><a href="javascript:;" class="flbc" onclick="drag.closeStyleEdit(\''+id+'\');return false;" title="Close">\n'
/*vot*/			+'Close</a></span></h3>';
/*vot*/		var f = '<p class="o pns"><button onclick="drag.saveStyle(\''+id+'\');drag.closeStyleEdit(\''+id+'\');" class="pn pnc" value="true">\n\
			<strong>Submit</strong></button><button onclick="drag.closeStyleEdit(\''+id+'\')" class="pn" value="true"><strong>Cancel</strong></button></p>';
		this.removeMenu(e);
...
/*vot*/		var titlename = obj instanceof Block ? 'Block' : 'Frame';
/*vot*/		var repeatarr = [{'key':'Repeat','value':'repeat'},{'key':'No repeat','value':'no-repeat'},{'key':'Repeat Horizontally','value':'repeat-x'},{'key':'Repeat Vertically','value':'repeat-y'}];
...
/*vot*/		common += '<tr><th>Background Image:</th><td><input type="text" id="titleBgImage" class="px p_fre" value="'+bgimage+'" /> <select class="ps vm" id="titleBgRepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
/*vot*/		common += '<tr><th>Background Color:</th><td><input type="text" id="titleBgColor" class="px p_fre" value="'+bgcolor+'" size="7" />';
		common += getColorPalette(fid+'bgPalette_0', 'titleBgColor' ,bgcolor)+'</td></tr>';
		if (obj instanceof Tab) {
/*vot*/			var switchArr = [{'key':'onClick','value':'click'},{'key':'onMouseover','value':'mouseover'}];
			var switchType = obj.titles['switchType'] ? obj.titles['switchType'][0] : 'click';
/*vot*/			common += '<tr><th>Switch Type:</th><td><select class="ps" id="switchType" >'+this.getOption(switchArr,switchType)+'</select></td></tr>';
		}
		common += '</table><hr class="l">';
		var li = '';
/*vot*/		li += '<div id="titleInput_0"><table class="tfm"><tr><th>'+titlename+' Title:</th><td><input type="text" id="titleText_0" class="px p_fre" value="`title`" /></td></tr>';
/*vot*/		li += '<tr><th>Link:</th><td><input type="text" id="titleLink_0" class="px p_fre" value="`link`" /></td></tr>';
/*vot*/		li += '<tr><th>Image:</th><td><input type="text" id="titleSrc_0" class="px p_fre" value="`src`" /></td></tr>';
/*vot*/		li += '<tr><th>Position:</th><td><select id="titleFloat_0" class="ps vm"><option value="" `left`>Align Left</option><option value="right" `right`>Align Right</option></select>';
/*vot*/		li += '&nbsp;&nbsp;Offset:<input type="text" id="titleMargin_0" class="px p_fre vm" value="`margin`" size="2" />px</td></tr>';
/*vot*/		li += '<tr><th>Font:</th><td><select class="ps vm" id="titleSize_0" ><option value="">Size</option>`size`</select>';
/*vot*/		li += '&nbsp;&nbsp;Color: <input type="text" id="titleColor_0" class="px p_fre vm" value="`color`" size="4" />';
		...
		var c = len + 1;
/*vot*/		html = '<div class="c diywin" style="width:450px;height:400px; overflow:auto;"><table cellspacing="0" cellpadding="0" class="tfm pns"><tr><th></th><td><button type="button" id="addTitleInput" class="pn" onclick="drag.addTitleInput('+c+');"><em>Add New Title</em></button></td></tr></table><div id="titleEdit">'+html+common+'</div></div>';
/*vot*/		var h = '<h3 class="flb"><em>Edit '+titlename+' Title</em><span><a href="javascript:;" class="flbc" onclick="drag.closeTitleEdit(\''+fid+'\');return false;" title="Close">\n' +
/*vot*/			'Close</a></span></h3>';
/*vot*/		var f = '<p class="o pns"><button onclick="drag.saveTitleEdit(\''+fid+'\');drag.closeTitleEdit(\''+fid+'\');" class="pn pnc" value="true">\n' +
/*vot*/			'<strong>Submit</strong></button><button onclick="drag.closeTitleEdit(\''+fid+'\')" class="pn" value="true"><strong>Cancel</strong></button></p>';
		this.removeMenu(e);
	...
	removeBlock : function (e, flag) {
		...
		var obj = this.getObjByName(id);
		if (!flag) {
/*vot*/			if (!confirm('Are you sure to delete it? It can not be restored if you delete it.')) return false;
		}
		if (obj instanceof Block) {
			this.delBlock(id);
		} else if (obj instanceof Frame) {
...
/*vot*/		bcontent.innerHTML = '<center>Loading content...</center>';
		var x = new Ajax();
		x.get('portal.php?mod=portalcp&ac=block&op=getblock&forceupdate=1&inajax=1&bid='+bid+'&tpl='+document.diyform.template.value, function(s) {
			if(s.indexOf('errorhandle_') != -1) {
				...
				runslideshow();
/*vot*/				showDialog('Sorry, you have no permission to add or edit block', 'alert');
				doane();
			} else {
	...
	frameExport : function (e) {
		var flag = true;
		if (drag.isChange) {
/*vot*/			flag = confirm('You have made some modifications, please import it after you save it, otherwise the imported data won\'t include modification of this time.');
		}
		if (flag) {
			if ( typeof e == 'object') {
				e = Util.event(e);
	...
	endBlockForceUpdateBatch : function () {
		if($('allupdate')) {
/*vot*/			$('allupdate').innerHTML = 'Updating is completed.';
	...
	getBlocks : function () {
		...
		if (this.blocks.length > 0) {
			...
			if($('allupdate')) {
/*vot*/				$('allupdate').innerHTML = 'Total <span style="color:blue">'+this.blocksLen+'</span> blocks, updating # <span style="color:red">'+cur+'</span> blocks, done <span style="color:red">'+(parseInt(cur / this.blocksLen * 100)) + '%</span>';
...
/*vot*/		showDialog('<div id="allupdate" style="width:350px;line-height:28px;">Start Updating...</div>','confirm','Updating block data', '', true, 'drag.endBlockForceUpdateBatch()');
		var wait = function() {
	...
	clearAll : function () {
/*vot*/		if (confirm('Are you sure to clear the current page DIY data? It can not be restored if you clear it.')) {
...
/*vot*/		if (objType == 'block' && !this.checkHasFrame()) {alert('Warning: Frame not found, please add frame.');spaceDiy.getdiy('frame');return false;}
		e = Util.event(e);
...
/*vot*/		str += '<div id="'+id+'_title" class="'+this.titleClass+' '+this.frameTitleClass+'"><span class="'+this.titleTextClass+'">'+type+' Frame</span></div>';
		var cols = type.split('-');
		var clsl='',clsc='',clsr='';
		clsl = ' frame-'+type+'-l';
		clsc = ' frame-'+type+'-c';
	...
	getTabHtml : function () {
		...
		var str = '<div id="'+id+'" class="'+className+'">';
/*vot*/		str += '<div id="'+id+'_title" class="'+titleClassName+'"><span class="'+this.titleTextClass+'">Tab Label</span></div>';
...
/*vot*/					return 'You have modified the data. If you exit, all the changes will be lost.';
				};
			}
			this.isChange = true;
	...
	goonDIY : function () {
		if ($('prefile').value == '1') {
/*vot*/			showDialog('<div style="line-height:28px;">Click the "Continue" button to load the temporary data into current style,<br />Click the "Delete" button for delete temporary data.</div>','confirm','Continue to DIY with temporary data?', function(){location.replace(location.href+'&preview=yes');}, true, 'spaceDiy.cancelDIY()', '', 'Continue', 'Delete');
...
/*vot*/					showDialog('<form name="selectsave" action="" method="get"><label><input type="radio" value="0" name="savemod"'+schecked+' />Apply to all this type pages</label>\n\
					<label><input type="radio" value="1" name="savemod"'+dchecked+' />Apply to current page</label></form>','notice', '', spaceDiy.save);
					return false;
				}
...
/*vot*/			showDialog('<div style="line-height:28px;">Save temporary data?<br />Click submit to save the temporary data, click cancel to delete the temporary data.</div>','confirm','Save the temporary data', function(){location.href = spaceDiy.cancelDiyUrl();}, true, function(){window.onunload=function(){spaceDiy.cancelDIY()};location.href = spaceDiy.cancelDiyUrl();});
		} else {
	...
	recover : function() {
/*vot*/		if (confirm('Are you sure you want to revert to previous version of saved results?')) {
			drag.clearClose();
