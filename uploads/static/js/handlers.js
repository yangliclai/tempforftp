...
function fileQueued(file) {
	try {
		...
		if(createQueue) {
/*vot*/			progress.setStatus('Loading ...');
		} else {
...
/*vot*/			showDialog("You have selected too many files.\n"+(message === 0 ? 'You cannot add any more files.' : 'You yet can choose ' + message + ' files'), 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			return;
		}
...
/*vot*/				progress.setStatus('File is too large.');
				this.debug("Error Code: File too big, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
/*vot*/				progress.setStatus('You can not upload a zero size file.');
				this.debug("Error Code: Zero byte file, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
/*vot*/				progress.setStatus('Upload of such type files is disabled.');
				this.debug("Error Code: Invalid File Type, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
/*vot*/				alert('You have selected too many files.\n' + (message > 1 ? 'You yet can choose ' +  message + ' files' : 'You cannot add any more files.'));
				break;
			default:
				if (file !== null) {
/*vot*/					progress.setStatus('Unhandled Error');
				}
...
function uploadStart(file) {
	try {
		...
		var progress = new FileProgress(file, this.customSettings.progressTarget);
/*vot*/		progress.setStatus('Uploading...');
		progress.toggleCancel(true, this);
		if(this.customSettings.uploadSource == 'forum') {
			var objId = this.customSettings.uploadType == 'attach' ? 'attachlist' : 'imgattachlist';
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	...
	try {
		...
		var progress = new FileProgress(file, this.customSettings.progressTarget);
/*vot*/		progress.setStatus("Uploaded ("+percent+"%)...");
...
/*vot*/						progress.setStatus('Cancelled');
					}
					this.cancelUpload(file.id);
					progress.setCancelled();
					progress.toggleCancel(true, this);
			...
			if(parseInt(data.picid)) {
				...
				newTd = document.createElement("TD");
				newTd.className = 'd';
/*vot*/				newTd.innerHTML = 'File Description<br/><textarea name="title['+data.picid+']" cols="40" rows="2" class="pt"></textarea>';
				newTr.appendChild(newTd);
				this.customSettings.imgBoxObj.appendChild(newTr);
			} else {
/*vot*/				showDialog('Image upload failed', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			}
			$(file.id).style.display = 'none';
		} else if(this.customSettings.uploadType == 'blog') {
			var data = eval('('+serverData+')');
...
/*vot*/				showDialog('Upload failed', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
/*vot*/				progress.setStatus('Cancelled');
				this.cancelUpload(file.id);
				progress.setCancelled();
				progress.toggleCancel(true, this);
...
/*vot*/			progress.setStatus('Upload completed.');
			progress.toggleCancel(false);
		}
	} catch (ex) {
		this.debug(ex);
