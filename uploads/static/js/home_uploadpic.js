...
/*vot*/		alert('Sorry, image with such extension does not supported');
		return;
	}
...
/*vot*/		localfile += '&nbsp;<a href="javascript:;" class="xi2" title="Click here to insert into the content at current cursor position" onclick="insertAttachimgTag(' + id + ');return false;">[Insert]</a>';
	}
	inhtml += '<td><strong>' + localfile +'</strong>';
/*vot*/	inhtml += '</td><td class="d">Image Description<br/><textarea name="pic_title" cols="40" rows="2" class="pt"></textarea>';
/*vot*/	inhtml += '</td><td class="o"><span id="showmsg' + id + '"><a href="javascript:;" onclick="delAttach(' + id + ');return false;" class="xi2">[Delete]</a></span>';
	inhtml += '</td></tr></table>';
...
function upload() {
	...
	if(nowUid>0) {
		...
		if(uploadStat==1) {
/*vot*/			upobj.innerHTML = 'Uploaded Successfully';
			successState = true;
			var InputNode;
			try {
...
/*vot*/			upobj.innerHTML = 'Upload Failed: '+uploadStat;
		}
	}

	if($('showmsg'+nid) != null) {
/*vot*/		$('showmsg'+nid).innerHTML = 'Uploading, Please wait (<a href="javascript:;" onclick="forms[nowUid].submit();">Retry</a>)';
