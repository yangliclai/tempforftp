...
/*vot*/				selector += '<option value="'+i+'" '+ (i == curpage ? 'selected' : '') +'>First '+i+' pages</option>';
			}
			selector += '</select>';
/*vot*/			selector += '<span>First '+curpage+' pages</span>';
		}

/*vot*/		$('div.pg').removeClass('pg').addClass('page').html('<a href="'+ prevpagehref +'">Prev</a>'+ selector +'<a href="'+ nextpagehref +'">Prev</a>');
...
var img = {
	...
	errorhandle : function(obj, is_err_t) {
		...
		if(is_err_t) {
			var parentnode = obj.parent();
			parentnode.find('.loading').remove();
/*vot*/			parentnode.append('<div class="error_text">Click to reload</div>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			$('#ntcmsg').remove();
			if(type == 'alert') {
/*vot*/				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="button2" type="button" value="Submit" onclick="popup.close();"></dd></div>'
			} else if(type == 'confirm') {
/*vot*/				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="redirect button2" type="button" value="Submit" href="'+ url +'"><a href="javascript:;" onclick="popup.close();">Cancel</a></dd></div>'
			}
...
var geo = {
	...
	locationerror : function(error) {
		...
		switch(error.code) {
			case error.TIMEOUT:
/*vot*/				geo.errmsg = 'Get location timeout, please try again';
				break;
			case error.POSITION_UNAVAILABLE:
/*vot*/				geo.errmsg = 'Unable to detect your current location';
			    break;
		    case error.PERMISSION_DENIED:
/*vot*/		        geo.errmsg = 'Please allow normal access to your current location';
		        break;
		    case error.UNKNOWN_ERROR:
/*vot*/		        geo.errmsg = 'Unknown error occurred';
		        break;
		}
	},
...
/*vot*/				contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_arrow.gif" style="vertical-align:middle;margin-right:5px;-moz-transform:rotate(180deg);-webkit-transform:rotate(180deg);-o-transform:rotate(180deg);transform:rotate(180deg);"><span id="refreshtxt">Touch down to refresh</span>');
...
/*vot*/						contentobj.find('#refreshtxt').html('Touch down to refresh');
					}
					reloadflag = false;
...
/*vot*/						contentobj.find('#refreshtxt').html('Touch UP to refresh');
					}
					reloadflag = true;
				}
			}
		...
		.on('touchend', function(e) {
			if(status == true) {
				if(reloadflag) {
/*vot*/					contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_load.gif" style="vertical-align:middle;margin-right:5px;">Loading...');
