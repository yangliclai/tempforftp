...
var ATTACHORIMAGE = '0';
var STATUSMSG = {
/*vot*/	'-1' : 'Internal Server Error',
/*vot*/	'0' :  'Uploaded Successfully',
/*vot*/	'1' :  'This file extension is not supported',
/*vot*/	'2' :  'Attachment size exceeds the allowed limit',
/*vot*/	'3' :  'You user group total attachment size exceeds allowed limit',
/*vot*/	'4' :  'This file extension is not supported',
/*vot*/	'5' :  'This file type total attachment size exceeds allowed limit',
/*vot*/	'6' :  'Daily total attachment size exceeds allowed limit',
/*vot*/	'7' :  'Select image files (' + imgexts + ')',
/*vot*/	'8' :  'Can not save Attachment file',
/*vot*/	'9' :  'No legitimate file was uploaded',
/*vot*/	'10' : 'Illegal Operation',
/*vot*/	'11' : 'Today you can not upload so much attachments'
};
...
/*vot*/		showError('Enter the title or content');
		return false;
/*vot*/	} else if(theform.subject.value.length > 80) {
/*vot*/		showError('Title length exceeds the limit of 255 characters');
		return false;
	}
...
/*vot*/			showError('Select a corresponding category for the thread');
			return false;
		}
		if(theform.sortid && (theform.sortid.options && theform.sortid.options[theform.sortid.selectedIndex].value == 0) && sortrequired) {
/*vot*/			showError('Select a corresponding category for the thread information');
			return false;
		}
	}
...
/*vot*/		showError('The content length does not meet the requirements.\n\nCurrent Length: ' + mb_strlen(message) + ' characters\nSystem limit: ' + postminchars + ' ~ ' + postmaxchars + ' characters');
		return false;
	}
	if(UPLOADSTATUS == 0) {
/*vot*/		if(!confirm('There are pending attachments, are you sure to ignore it?')) {
			return false;
		}
	} else if(UPLOADSTATUS == 1) {
/*vot*/		showDialog('Some attachments are still uploading, please wait. The thread will be published automaticly after the files was uploaded...', 'notice');
		AUTOPOST = 1;
		return false;
	}
...
/*vot*/		if(confirm('Because of you have set read permission or sell the post, do you confirm also the broadcast to your listeners to see?') == false) {
			return false;
		}
	}
...
/*vot*/					showError('Wrong answer, please try again');
					chk = 0;
				}
			}
			if(seccodecheck) {
...
/*vot*/					showError('Wrong security code, please try again');
					chk = 0;
				}
			}
			if(chk) {
...
function uploadNextAttach() {
	...
	var sizelimit = '';
	if(arr[4] == 'ban') {
/*vot*/		sizelimit = '(This attachment type is disabled)';
	} else if(arr[4] == 'perday') {
/*vot*/		sizelimit = '(Not larger than ' + arr[5] + ' characters)';
	} else if(arr[4] > 0) {
/*vot*/		sizelimit = '(Not larger than ' + arr[4] + ' characters)';
	}
...
function uploadAttach(curId, statusid, prefix, sizelimit) {
	...
	if(curId > 0) {
		...
		if(nextId == curId || in_array(statusid, [6, 8])) {
			...
			if(UPLOADFAILED > 0) {
/*vot*/				showDialog('Uploading is finished! Successfull: ' + UPLOADCOMPLETE + ', Failed: ' + UPLOADFAILED + ', items: ' + FAILEDATTACHS);
				FAILEDATTACHS = '';
			}
			UPLOADSTATUS = 2;
			for(var i = 0; i < AID[prefix ? 1 : 0] - 1; i++) {
...
/*vot*/				showDialog('Uploading is finished!', 'right', null, null, 0, null, null, null, null, 3);
			}
...
/*vot*/	$(prefix + 'cpdel_' + nextId).innerHTML = '<img src="' + IMGDIR + '/loading.gif" alt="Uploading..." />';
	UPLOADSTATUS = 1;
...
/*vot*/		showError('Sorry, such file extension does not supported.');
		return;
	}
	if(prefix == 'img' && imgexts.indexOf(ext) == -1) {
		reAddAttach(prefix, id);
/*vot*/		showError('Select image files (' + imgexts + ')');
		return;
	}

/*vot*/	$(prefix + 'cpdel_' + id).innerHTML = '<a href="javascript:;" class="d" onclick="reAddAttach(\'' + prefix + '\', ' + id + ')">Delete</a>';
...
/*vot*/		objupdate.innerHTML = '<input type="file" name="attachupdate[paid' + aid + ']"><a href="javascript:;" onclick="updateAttach(' + aid + ')">Cancel</a>';
	} else {
		obj.style.display = '';
		objupdate.innerHTML = '';
	}
...
function updateattachnum(type) {
	...
	if(num) {
		if($(editorid + '_' + type)) {
/*vot*/			$(editorid + '_' + type).title = 'Contains ' + num + (type == 'image' ? ' images attached' : ' files attached');
		}
		...
		if($(editorid + '_' + type)) {
/*vot*/			$(editorid + '_' + type).title = type == 'image' ? 'Images' : 'attachments';
		}
		if($(editorid + '_' + type + 'n')) {
...
function updateDownImageList(msg) {
	...
	if(msg == '') {
/*vot*/		showError('Sorry, no remote attachment');
	} else {
		...
		switchImagebutton('imgattachlist');$('imgattach_notice').style.display = '';
/*vot*/		showDialog('Remote attachment download is completed!', 'right', null, null, 0, null, null, null, null, 3);
	}
}
...
/*vot*/			sizelimit = '(Not larger than ' + arr[8] + ' characters)';
		} else if(arr[7] > 0) {
/*vot*/			sizelimit = '(Not larger than ' + arr[7] + ' characters)';
		}
...
/*vot*/		$('polloption_new').outerHTML = '<span>Reached the maximum number of votes: '+maxoptions+'</span>';
	}
}
...
/*vot*/			'<p class="o pns"><span class="z xg1"><label for="unusedwinchkall"><input id="unusedwinchkall" type="checkbox" onclick="attachoption(\'' + type + '\', 3)" checked="checked" />Select All</label></span>' +
/*vot*/			'<button onclick="attachoption(\'' + type + '\', 1);hideMenu(\'fwin_dialog\', \'dialog\')" class="pn pnc"><strong>OK</strong></button></p>', 'info', 'Unused ' + (type == 'attach' ? 'Attachment' : 'Image'));
	} else if(op == 3) {
...
/*vot*/		$('replycredit').innerHTML = '<b class="xi1">Reply Reward Points total is too large ('+real_reply_credit+')</b>';
	} else {
		if(have_replycredit > 0 && real_reply_credit < 0) {
/*vot*/			$('replycredit').innerHTML = "<font class='xi1'>Return "+Math.abs(real_reply_credit)+"</font>";
		} else {
...
/*vot*/	if(confirm('Sure you want to delete this post?') != 0){
		$('delete').value = '1';
		$('postform').submit();
	}
}
