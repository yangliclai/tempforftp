...
/*vot*/ var lastusername = '', lastpassword = '', lastemail = '', lastinvitecode = '', stmp = new Array(), modifypwd = false, profileTips = 'Leave blank if you do not need to change the password';
...
function checkPwdComplexity(firstObj, secondObj, modify) {
	...
	firstObj.onblur = function () {
		if(firstObj.value == '') {
/*vot*/			var pwmsg = !modifypwd ? 'Please fill the password' : profileTips;
			if(pwlength > 0) {
/*vot*/				pwmsg += ', Minimum length '+pwlength+' characters';
			}
...
/*vot*/			var passlevels = new Array('','Weak','Middle','Strong');
			var passlevel = checkstrongpw(firstObj.id);
/*vot*/			errormessage(firstObj.id, '<span class="passlevel passlevel'+passlevel+'">Password strength: ' + passlevels[passlevel]+'</span>');
		}
	};
	secondObj.onblur = function () {
		if(secondObj.value == '') {
/*vot*/			errormessage(secondObj.id, !modifypwd ? 'Please enter the password again' : profileTips);
		}
		checkpassword(firstObj.id, secondObj.id);
	};
}
...
function addMailEvent(mailObj) {
	...
	mailObj.onblur = function () {
		if(mailObj.value == '') {
/*vot*/			errormessage(mailObj.id, 'Please enter email address');
		}
...
function showbirthday(){
	...
	el.length=0;
/*vot*/	el.options.add(new Option('Day', ''));
	for(var i=0;i<28;i++){
...
/*vot*/		errormessage(id, 'User name contains invalid characters');
		return;
	}
...
/*vot*/	if(unlen < 2) {
/*vot*/		errormessage(id, 'User name is shorter than 3 characters');
		return;
/*vot*/	} else if(unlen > 15) {
		errormessage(id, 'User name is longer than 15 characters');
		return;
/*vot*/	} else if(utf8len > 64) { // Max DB Field Length in bytes
/*vot*/		errormessage(id, 'User name is longer than 64 bytes');
/*vot*/		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
/*vot*/			errormessage(id1, 'Password is too short, must be not less than '+pwlength+' characters');
			return;
		}
	}
	if(strongpw) {
		...
		for(var i in strongpw) {
			if(strongpw[i] === 1 && !$(id1).value.match(/\d+/g)) {
				strongpw_error = true;
/*vot*/				strongpw_str[j] = 'Digits';
				j++;
			}
			if(strongpw[i] === 2 && !$(id1).value.match(/[a-z]+/g)) {
				strongpw_error = true;
/*vot*/				strongpw_str[j] = 'Lowercase letters';
				j++;
			}
			if(strongpw[i] === 3 && !$(id1).value.match(/[A-Z]+/g)) {
				strongpw_error = true;
/*vot*/				strongpw_str[j] = 'Capital letters';
				j++;
			}
			if(strongpw[i] === 4 && !$(id1).value.match(/[^A-Za-z0-9]+/g)) {
				strongpw_error = true;
/*vot*/				strongpw_str[j] = 'Special symbols';
				j++;
			}
		}
		if(strongpw_error) {
/*vot*/			errormessage(id1, 'Weak password, the password must contain '+strongpw_str.join(', '));
			return;
		}
	}
	errormessage(id2);
	if($(id1).value != $(id2).value) {
/*vot*/		errormessage(id2, 'Two passwords does not match');
	} else {
...
function checkemail(id) {
	...
	if(email.match(/<|"/ig)) {
/*vot*/		errormessage(id, 'Email contains invalid characters');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
/*vot*/		errormessage('invitecode', 'Invitation code contains invalid characters');
		return;
	}
	var x = new Ajax();
