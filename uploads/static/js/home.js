...
/*vot*/		alert('Category name can not be empty!');
		return false;
	}
}
...
/*vot*/					this.title = 'Show original image in a new window';
...
/*vot*/			if(confirm('Are you sure to proceed?')) {
				return true;
			} else {
				return false;
			}
...
/*vot*/	alert('Please choose the item to operate with');
	return false;
}
...
function showFlash(host, flashvar, obj, shareid) {
	...
	var videoMp3 = true;
	if('' == flashvar) {
/*vot*/		alert('Incorrect audio URL, can not be empty');
		return false;
	}
	if('music' == host) {
...
/*vot*/		var nodetxt = document.createTextNode('Collapse');
...
/*vot*/		$('a_app_more').innerHTML = 'Collapse';
...
/*vot*/		$('a_app_more').innerHTML = 'Expand';
		$('a_app_more').onclick = function() {
			userapp_open();
		};
	});
...
function myfriend_post(uid) {
	if($('friend_'+uid)) {
/*vot*/		$('friend_'+uid).innerHTML = '<p>You are friends now, you can <a href="home.php?mod=space&do=wall&uid='+uid+'" class="xi2" target="_blank">Leave a message</a>, or <a href="home.php?mod=spacecp&ac=poke&op=send&uid='+uid+'&handlekey=propokehk_'+uid+'" id="a_poke_'+uid+'" class="xi2" onclick="showWindow(this.id, this.href, \'get\', 0, {\'ctrlid\':this.id,\'pos\':\'13\'});">send greeting</a></p>';
	}
	showCreditPrompt();
}
...
/*vot*/	$(opid).innerHTML = '&laquo; Close the List';
	$(opid).onclick = function() {
...
/*vot*/	$(opid).innerHTML = '&raquo; More Feeds';
	$(opid).onclick = function() {
